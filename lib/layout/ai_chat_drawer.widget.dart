...
/// Pannello AI chat inline (senza Drawer), usato nel layout desktop.
    ...
    return Container(
      ...
      child: Row(
        children: [
          Container(
            padding: const EdgeInsets.all(8),
            decoration: BoxDecoration(color: theme.primary.withValues(alpha: 0.1), borderRadius: BorderRadius.circular(Sizes.borderRadius)),
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('Assistente AI', style: theme.title.copyWith(fontSize: 15)),
                if (_controller != null && _controller!.isProcessing)
                  Text('Sta pensando...', style: theme.smallLabel.copyWith(color: theme.primary))
                else
                  Text('Chiedimi qualcosa', style: theme.smallLabel),
              ],
            ),
          ...
          IconButton(
            icon: HugeIcon(icon: HugeIcons.strokeRoundedCancel01, color: theme.secondaryText, size: 18),
            onPressed: () => context.read<AppState>().toggleAiChat(),
            padding: EdgeInsets.zero,
            constraints: const BoxConstraints(minWidth: 36, minHeight: 36),
          ),
        ],
      ...
      return Center(
        child: Padding(
          ...
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              HugeIcon(icon: HugeIcons.strokeRoundedAiChat02, color: theme.secondaryText.withValues(alpha: 0.4), size: 48),
              const SizedBox(height: Sizes.padding),
              Text('Assistente AI', style: theme.heading6.copyWith(color: theme.secondaryText)),
              const SizedBox(height: 8),
              Text('Chiedimi di navigare, trovare informazioni o eseguire azioni nell\'app.',
                  style: theme.bodyLabel.copyWith(color: theme.secondaryText), textAlign: TextAlign.center),
            ],
          ),
        ),
      );
          ...
          return Padding(
            ...
            child: Row(
              ...
              children: [
                Container(
                  width: 28, height: 28,
                  decoration: BoxDecoration(color: theme.primary.withValues(alpha: 0.12), borderRadius: BorderRadius.circular(8)),
                  child: Icon(Icons.auto_awesome, size: 16, color: theme.primary),
                ),
                const SizedBox(width: 8),
                Flexible(
                  child: Container(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        const SizedBox(width: 10),
                        Flexible(child: Text(progressText ?? 'Sto elaborando...', style: theme.bodyLabel.copyWith(color: theme.secondaryText, fontSize: 13))),
                      ],
                    ),
                  ),
                ),
            ...
            children: [
              ...
              Flexible(
                child: Container(
                  padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 10),
                  decoration: BoxDecoration(
                    color: isUser ? theme.primary.withValues(alpha: isDark ? 0.25 : 0.12) : (isDark ? theme.primaryBackground : theme.primaryBackground.withValues(alpha: 0.7)),
                  ...
                  child: SelectableText(msg.content, style: theme.bodyText.copyWith(fontSize: 13.5, height: 1.45, color: theme.primaryText)),
                ),
              ),
              if (isUser) const SizedBox(width: 8),
            ],
