...
class AnalogClockPainter extends CustomPainter {
 ...
 final bool showDigitalClock;
  ...
  void _drawIndicators(Canvas canvas, Size size, double scaleFactor, bool showAllNumbers) {
    TextStyle style = TextStyle(color: numberColor, fontSize: 18.0 * scaleFactor * textScaleFactor);
    double p = 12.0;
    if (showTicks) p += 24.0;
  ...
  void _paintDigitalClock(Canvas canvas, Size size, double scaleFactor) {
    String textToBeDisplayed =
        (!(format?.isEmpty ?? true))
            ...
            : showDate
            ? INTL.DateFormat('dd/MM/yyyy HH:mm:ss').format(datetime)
            : showSeconds
            ? INTL.DateFormat('HH:mm:ss').format(datetime)
            : INTL.DateFormat('HH:mm').format(datetime);
    TextSpan digitalClockSpan = TextSpan(
      style: textStyle ?? TextStyle(color: digitalClockTextColor, fontSize: 18 * scaleFactor * textScaleFactor),
