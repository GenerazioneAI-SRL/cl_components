...
class GenaiAiAssistantController extends ChangeNotifier {
  final GenaiAiAssistantConfig _config;
  ...
  /// Emit an analytics event to the host app's [GenaiAiAssistantConfig.onEvent].
  void _emit(AiEventType type, [Map<String, dynamic>? properties]) {
    _config.onEvent?.call(AiEvent.now(type, properties));
  }

  GenaiAiAssistantController({
    required GenaiAiAssistantConfig config,
  ...
  GenaiAiAssistantConfig get config => _config;
  ...
  void _init() {
    AiLogger.log('Initializing GenaiAiAssistantController', tag: 'Controller');
    // 1. Context engine.
  ...
  @override
  void dispose() {
    AiLogger.log('Disposing GenaiAiAssistantController', tag: 'Controller');
    _disposed = true;
    _cancelRequested = true;
    _processingTimer?.cancel();
    _responsePopupTimer?.cancel();
