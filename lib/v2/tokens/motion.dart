...
/// A (Duration, Curve) pair — the unit of motion in v2 (§3.6).
///
/// Every animated interaction reads from a [GenaiMotion] token so timing and
/// easing stay consistent across the product. `reduced()` returns a zero-
/// duration variant for users who enable the OS "reduce motion" setting.
@immutable
class GenaiMotion {
...
/// Semantic motion tokens — v2 design system (§3.6).
///
/// Seven canonical motions drive every animation in the product. Mirror of
/// the §3.6 table exactly:
///
/// | Token    | Duration | Easing        |
/// |----------|----------|---------------|
/// | hover    | 120 ms   | ease-out      |
/// | press    | 80 ms    | ease-out      |
/// | expand   | 220 ms   | emphasized    |
/// | modal    | 240 ms   | emphasized    |
/// | toast    | 200 ms   | ease-out      |
/// | page     | 180 ms   | ease-in-out   |
/// | spring   | 400 ms   | spring        |
@immutable
class GenaiMotionTokens {
  /// Color/background hover transitions — 120 ms, ease-out.
  final GenaiMotion hover;

  /// Press scale-down / up — 80 ms, ease-out.
  final GenaiMotion press;

  /// Accordion / collapsible expand — 220 ms, emphasized.
  final GenaiMotion expand;

  /// Modal / drawer open/close — 240 ms, emphasized.
  final GenaiMotion modal;

  /// Toast enter/exit — 200 ms, ease-out.
  final GenaiMotion toast;

  /// Route transition — 180 ms, ease-in-out.
  final GenaiMotion page;

  /// Personality spring (AI reveal, delight) — 400 ms, spring.
  ...
  /// Default motion tokens per §3.6 table.
  ...
  /// All-zero-duration variant for `prefers-reduced-motion` users (§3.6).
  ...
  /// Motion is categorical — `lerp` snaps at midpoint to avoid intermediate
  /// curves / sub-frame durations.
