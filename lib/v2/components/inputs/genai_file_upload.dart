...
/// Metadata for a single uploaded file — rendered as a row below the drop
/// zone. Consumers choose whatever transport they like (multipart, direct
/// upload, AI ingest) and merely feed progress + status back.
@immutable
class GenaiUploadedFile {
  ...
  /// Error copy — shown instead of progress when status == error.
...
/// Drop-zone + file-list file uploader.
///
/// v2 keeps this widget UI-only: it calls [onPickFiles] when the user clicks
/// the zone or drops files (drop-detection is web-only and left to the host
/// app). The host feeds back [files] with any progress/error metadata.
class GenaiFileUpload extends StatefulWidget {
  ...
  /// Callback for when the user clicks the zone.
  final VoidCallback? onPickFiles;

  /// Callback when the user removes a file row.
  final ValueChanged<int>? onRemove;

  /// Field label above the zone.
  final String? label;

  /// Placeholder inside the zone — defaults to standard copy.
...
class _GenaiFileUploadState extends State<GenaiFileUpload> {
  ...
  Widget build(BuildContext context) {
        ...
        child: GestureDetector(
          ...
          child: AnimatedContainer(
            ...
            decoration: BoxDecoration(
              color:
                  widget.isDisabled ? colors.surfaceHover : colors.surfaceInput,
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  widget.hintText ?? 'Click to upload or drop files here',
    ...
    return Semantics(
      button: true,
      label: widget.semanticLabel ?? widget.label ?? 'File upload',
