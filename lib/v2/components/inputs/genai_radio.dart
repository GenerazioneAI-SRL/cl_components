...
/// A single option rendered inside a [GenaiRadio] group.
class GenaiRadioOption<T> {
  /// The value used by the group to identify the option.
  final T value;
  ...
  /// When true, this individual option is disabled regardless of group state.
...
/// Single-choice radio group for v2 forms.
///
/// Renders a vertical (default) or horizontal set of [GenaiRadioOption]s
/// with a shared [value] + [onChanged] pair. Options announce themselves
/// as being in a mutually-exclusive group for assistive tech.
class GenaiRadio<T> extends StatelessWidget {
  /// Currently selected value; null means nothing selected.
  final T? value;
  ...
  /// Error state — red outer ring on all options.
  final bool hasError;
