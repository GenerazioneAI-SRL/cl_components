...
/// On/off switch-style toggle for v2 forms (distinct from
/// `GenaiToggleButton` in actions/ which is a button-shaped toggle).
///
/// Track pill slides the thumb from left (off) to right (on). Optional
/// [label] + [description] on the left, toggle on the right.
class GenaiToggle extends StatefulWidget {
  /// Current state.
  final bool value;

  /// Change callback; receives the new state. `null` disables the control.
  final ValueChanged<bool>? onChanged;

  /// Inline label rendered to the left of the track.
  final String? label;

  /// Secondary description shown below [label].
...
class _GenaiToggleState extends State<GenaiToggle> {
  ...
  // Normal-density dimensions. Thumb padding keeps 2 px margin.
