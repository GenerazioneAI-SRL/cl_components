...
/// Tri-state checkbox for v2 forms.
///
/// [value]:
/// * `true`  — checked
/// * `false` — unchecked
/// * `null`  — indeterminate (minus glyph)
///
/// Tapping transitions `null → true → false → true → …`. Consumers driving
/// the indeterminate state externally should supply their own [onChanged]
/// handler that cycles values as required.
class GenaiCheckbox extends StatefulWidget {
  /// Pass `null` for the indeterminate state.
  final bool? value;

  /// Callback fired with the new value. `null` disables the checkbox.
  ...
  /// Disabled — muted colour, no interaction.
  final bool isDisabled;

  /// Error state — red ring, red focus override.
  final bool hasError;
...
class _GenaiCheckboxState extends State<GenaiCheckbox> {
  ...
  // 18 px box — matches default density iconSize.
  ...
  Widget build(BuildContext context) {
    ...
    final restingBorder =
        widget.hasError ? colors.colorDanger : colors.borderStrong;
    final hoverBorder =
        widget.hasError ? colors.colorDangerHover : colors.textSecondary;
