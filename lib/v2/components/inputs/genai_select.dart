...
/// A single selectable option in a [GenaiSelect].
...
/// Dropdown single-select for v2 forms.
///
/// Presents a trigger shaped like [GenaiTextField] with a chevron affordance.
/// Tapping opens an overlay containing the option list; an optional search
/// box filters the list when [searchable] is true.
class GenaiSelect<T> extends StatefulWidget {
  /// Currently selected value (must match one of [options]' values or be null).
  final T? value;

  /// Available options.
  final List<GenaiSelectOption<T>> options;

  /// Callback fired when the user picks an option.
  ...
  /// When true, shows a search box in the overlay for filtering.
...
class _GenaiSelectState<T> extends State<GenaiSelect<T>> {
  ...
  Widget _buildOverlay(BuildContext overlayContext) {
    // Use the outer widget's BuildContext for tokens so theme matches.
...
/// Internal shared row that renders a select/combobox option.
...
/// Internal shared search row used in select/combobox overlays.
