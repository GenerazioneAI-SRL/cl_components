...
/// Swatch-palette colour picker with optional hex input.
///
/// Presents a grid of [swatches] the user can tap to select. When
/// [showHexInput] is true, a small text box beneath the grid allows typing
/// an arbitrary `#RRGGBB` value (3- or 6-digit).
class GenaiColorPicker extends StatefulWidget {
  /// Current colour (null = unset).
  final Color? value;

  /// Fired when the user picks / types a new colour.
  final ValueChanged<Color>? onChanged;

  /// Palette swatches. Defaults to a sensible editorial palette.
...
class _GenaiColorPickerState extends State<GenaiColorPicker> {
  ...
  Widget build(BuildContext context) {
    ...
    return Semantics(
      label: widget.semanticLabel ?? widget.label ?? 'Colour picker',
      enabled: !widget.isDisabled,
      child: FieldFrame(
        label: widget.label,
