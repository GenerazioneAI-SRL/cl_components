...
/// One-time-password input (N-digit pin).
///
/// Renders [length] separate square boxes; typing auto-advances focus, and
/// backspace on an empty box walks back. [onCompleted] fires when all [length]
/// slots are filled.
class GenaiOtpInput extends StatefulWidget {
  ...
  /// Change callback — emitted on every keystroke.
  final ValueChanged<String>? onChanged;

  /// Completion callback — emitted when the value reaches [length] characters.
  final ValueChanged<String>? onCompleted;

  /// Field label above the boxes.
  final String? label;

  /// Helper copy under the boxes.
...
class _GenaiOtpInputState extends State<GenaiOtpInput> {
  ...
  void _onSlotChanged(int index, String raw) {
    // Accept only the last typed char — if user pasted, distribute.
    String v = raw;
    if (widget.digitsOnly) {
      v = v.replaceAll(RegExp(r'[^0-9]'), '');
    }
    if (v.length > 1) {
      // Pasted — distribute across slots from [index].
