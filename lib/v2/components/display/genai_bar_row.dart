...
/// Horizontal bullet bar row — v2 design system.
///
/// Dashboard primitive: a compact 3-column row that renders a labelled bar
/// with a right-aligned tabular value. Mirrors the `.bar-row` component in
/// the Forma LMS reference HTML.
///
/// - Label column: 100 px, `bodySm` primary text.
/// - Bar: rounded track (`borderDefault`) with a filled segment sized by
///   [value] (0..1) in [barColor] (defaults to `textPrimary`).
/// - Value column: 60 px, right-aligned, `monoSm` (tabular figures).
///
/// Callers build color semantics via [barColor] — pass `colorInfo`,
/// `colorSuccess`, `colorWarning`, or `colorDanger` for a semantic tone;
/// leave null for the neutral ink default.
class GenaiBarRow extends StatelessWidget {
  ...
  /// Accessible label. Defaults to `"$label $valueLabel"`.
