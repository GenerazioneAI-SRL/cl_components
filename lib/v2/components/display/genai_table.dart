...
/// Free-text filter backed by a plain Material [TextField] in v2.
///
/// Wave 2 will swap the editor for a native `GenaiTextField` once that
/// component lands.
...
/// Async data table — v2 design system.
///
/// Streamlined v2 renderer: header, toolbar, rows, bulk-action bar. Uses
/// Material `InkWell` / `TextField` fallbacks where v2 input components are
/// still pending — those editors are isolated so they can be replaced
...
class _GenaiTableState<T> extends State<GenaiTable<T>> {
  ...
  double _rowVerticalPadding(BuildContext context) {
    ...
    return switch (_density) {
      ...
      GenaiTableDensity.normal => s.s8,
      GenaiTableDensity.comfortable => s.s12,
    };
  }
  ...
  Widget _buildHeader(BuildContext context) {
    ...
    final s = context.spacing;
    return Padding(
      padding: EdgeInsets.fromLTRB(s.s16, s.s16, s.s16, 0),
      child: Row(
  ...
  Widget _buildTable(BuildContext context) {
      ...
      return Padding(
        ...
        child: Center(
          child: Text(
            ...
            style: ty.bodyMd.copyWith(color: colors.colorDangerText),
          ),
        ),
      );
    }
    ...
    return SingleChildScrollView(
      ...
      child: DataTable(
        ...
        columnSpacing: s.s24,
        horizontalMargin: s.s16,
        dividerThickness: sizing.dividerThickness,
        headingTextStyle: ty.labelMd.copyWith(color: colors.textSecondary),
    ...
    return InkWell(
      ...
      child: Row(
        ...
        children: [
          ...
          SizedBox(width: s.s4),
          Icon(icon,
              size: context.sizing.iconSize, color: colors.textSecondary),
        ],
      ),
    );
  }
