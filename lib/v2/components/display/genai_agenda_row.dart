...
/// Agenda / calendar date row — v2 design system.
///
/// Lists upcoming events with a fixed-width date tile on the left and a
/// flexible content column on the right. Mirrors the `.agenda-row` block:
/// - 58 px date tile (`surfaceHover` bg, `radius.lg` (12), padding 6/8).
///   Day uses `headingMd` (tabular figures); month uses `labelSm` uppercase.
/// - 1fr content: title (`labelMd` 600), subtitle (`labelSm` ink-2), optional
///   meta row of icon+text widgets.
/// - Row padding 16/20, divided from the next row by a `borderDefault` line.
///
