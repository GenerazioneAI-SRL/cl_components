...
enum GenaiSkeletonShape {
  ...
  /// Single line of text (height derived from bodyMd line height).
  text,
}

/// Loading placeholder — v2 design system (§6.6).
///
/// Uses a subtle shimmer tween between `surfaceHover` and `surfacePressed`.
/// Respects reduced-motion by rendering a static tinted rectangle.
...
class _GenaiSkeletonState extends State<GenaiSkeleton>
  ...
  Widget build(BuildContext context) {
    ...
    final double resolvedHeight = widget.height ??
        switch (widget.shape) {
          GenaiSkeletonShape.text => ty.bodyMd.fontSize ?? 14,
