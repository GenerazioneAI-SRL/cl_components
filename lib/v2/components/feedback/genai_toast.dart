...
import 'genai_alert.dart';

/// Toast anchor position — v2 design system (§6.4).
...
/// Inline toast notification — v2 design system (§6.4).
///
/// Used as the content of the overlay entry mounted by [showGenaiToast]; can
/// also be embedded directly for static previews (e.g. showcase).
class GenaiToast extends StatelessWidget {
  /// Severity — reuses [GenaiAlertType] mapping.
  ...
  ({Color fg, Color border, IconData icon}) _resolve(BuildContext context) {
...
///
/// Mounts a [GenaiToast] as an [OverlayEntry] anchored via [position].
/// Auto-dismisses after [duration] (default 4 s). When the user prefers
/// reduced motion, enter/exit transitions collapse to zero.
///
...
Future<void> showGenaiToast(
  ...
  // Wire Esc → dismiss at app level by attaching a HardwareKeyboard listener.
