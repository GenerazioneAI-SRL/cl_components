...
/// Single row in a menubar dropdown. Keep this v2-local so the navigation
/// cluster is self-contained (no dependency on the overlay cluster).
@immutable
class GenaiMenubarItem {
  ...
  /// Value returned via the `onSelected` callback when this item is picked.
  final Object? value;
...
class GenaiMenubarMenu {
  /// Trigger text shown in the menubar row (e.g. `File`, `Edit`).
  final String label;

  /// Items rendered inside the dropdown when this menu opens.
  final List<GenaiMenubarItem> items;

  /// Optional accessibility override for the trigger.
...
///
/// Renders a row of ghost-style triggers. Clicking a trigger opens its
/// dropdown. While a menu is open, hovering a sibling trigger swaps dropdowns
/// without requiring a click (v2 polish).
///
/// Keyboard model:
/// - Left/Right: move between top-level menus.
/// - Up/Down: move through items of the open menu.
/// - Enter/Space: activate the highlighted item.
/// - Esc: close the dropdown and return focus to the trigger.
class GenaiMenubar extends StatefulWidget {
  /// Top-level menus, rendered in order.
  final List<GenaiMenubarMenu> menus;

  /// Fires with the selected item's [GenaiMenubarItem.value].
...
class _GenaiMenubarState extends State<GenaiMenubar> {
  ...
  void _openMenu(int index) {
    ...
    // Hover-switching to a sibling: reuse the existing OverlayEntry instead
    // of remove+insert (which causes a one-frame blink).
    if (_entry != null) {
      setState(() {
        _openIndex = index;
    ...
    _entry = OverlayEntry(
      builder: (ctx) {
        ...
        return _MenubarOverlay(
          triggerOrigin:
              Offset(triggerTopLeft.dx, triggerTopLeft.dy + triggerSize.height),
          items: activeMenu.items,
          highlight: _highlightedItem,
          onHighlight: (idx) {
            if (_highlightedItem == idx) return;
            setState(() => _highlightedItem = idx);
            // setState rebuilds this State, but the OverlayEntry's builder
            // closure must be triggered explicitly or the highlight blinks.
            _entry?.markNeedsBuild();
          },
...
class _MenubarTriggerState extends State<_MenubarTrigger> {
  ...
  Widget build(BuildContext context) {
    ...
    // Focus ring as a Stack overlay to avoid layout shift on focus change.
    final stacked = Stack(
...
class _MenubarOverlayState extends State<_MenubarOverlay> {
  ...
  Widget build(BuildContext context) {
    ...
    return Stack(
      children: [
        // Backdrop that dismisses on outside tap.
        Positioned.fill(
          child: GestureDetector(
