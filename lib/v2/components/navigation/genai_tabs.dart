...
class GenaiTabItem {
  ...
  /// Optional leading icon (sized from `context.sizing.iconSize`).
  final IconData? icon;

  /// Optional numeric badge shown after the label.
  final int? badgeCount;

  /// When true the tab is non-interactive and rendered in the disabled
  /// color ramp.
...
/// Visual style of a [GenaiTabs] row (v2 §6.6.1).
enum GenaiTabsVariant {
  /// Thin 2 px underline below the selected tab (v2 default).
  underline,

  /// Filled pill for the selected tab.
  pill,

  /// iOS-like segmented control inside a rounded container.
  segmented,
}

/// Tabs — v2 design system.
///
/// Keyboard model: ←/→ cycle, Home/End jump to ends, Enter/Space activate.
/// Hover uses `context.colors.surfaceHover` (alpha tint per §4 polish).
class GenaiTabs extends StatefulWidget {
  /// Ordered tab list. Empty lists render nothing.
  final List<GenaiTabItem> items;

  /// Currently selected index. Values outside `0..items.length-1` select none.
  final int selectedIndex;

  /// Fires when a user activates a different tab.
  final ValueChanged<int>? onChanged;

  /// Visual style (§6.6.1). Defaults to [GenaiTabsVariant.underline].
  final GenaiTabsVariant variant;

  /// When true tabs expand to fill the row equally. Default: false.
...
class _GenaiTabsState extends State<GenaiTabs> {
  ...
  void _move(int delta) {
    ...
    // Skip disabled tabs in the direction of motion.
    var guard = 0;
  ...
  Widget build(BuildContext context) {
    ...
    // Decorate rows for underline/segmented variants.
...
/// A single interactive tab cell rendering icon + label + badge.
...
class _TabCellState extends State<_TabCell> {
  ...
  Widget build(BuildContext context) {
    ...
    final labelStyle = ty.labelLg.copyWith(color: fg);
    ...
    // Focus ring for keyboard-only focus.
...
/// Tiny count badge rendered inside a tab.
