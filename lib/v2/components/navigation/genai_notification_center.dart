...
/// Severity level of a [GenaiNotificationItem]. Drives icon and colour ramp.
...
/// Single entry inside a [GenaiNotificationCenter].
@immutable
class GenaiNotificationItem {
  /// Stable id used by the dismiss / read callbacks.
  final String id;
  ...
  /// Timestamp used for the relative-time label ("2 min ago").
  final DateTime timestamp;

  /// Severity.
  final GenaiNotificationLevel level;

  /// When true the entry is rendered as already-read (muted).
  final bool isRead;

  /// Fires when the entry is tapped (e.g. to deep-link).
...
/// Notification centre — v2 design system.
///
/// Hosted inside a drawer or popover. Title row shows unread count. Each
/// entry has a level-coloured dot, title, body, relative time, and optional
/// dismiss / mark-as-read affordances.
class GenaiNotificationCenter extends StatelessWidget {
  /// Notification list (newest first is the expected input order).
  final List<GenaiNotificationItem> notifications;

  /// Fires when the user taps the "mark all as read" affordance.
  final VoidCallback? onMarkAllRead;

  /// Fires with the item id when an entry is dismissed.
  final ValueChanged<String>? onDismiss;

  /// Fires with the item id when an entry is marked as read.
  final ValueChanged<String>? onMarkRead;

  /// Localised title. Defaults to `Notifications`.
  final String title;

  /// Localised empty-state copy.
  final String emptyLabel;

  /// Localised "mark all as read" label.
  final String markAllLabel;

  /// Accessibility override for the panel.
...
class _NotificationRowState extends State<_NotificationRow> {
  ...
  Widget build(BuildContext context) {
    ...
    final titleStyle = ty.labelMd.copyWith(
          ...
          child: Padding(
            ...
            child: Text(
              ...
              style: ty.labelMd.copyWith(
                color: _hover ? colors.colorPrimaryHover : colors.colorPrimary,
                decoration:
