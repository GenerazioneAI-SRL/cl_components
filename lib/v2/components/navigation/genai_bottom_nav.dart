...
class GenaiBottomNavItem {
  ...
  /// Optional red badge — when non-null an indicator is rendered over the
  /// icon; the number is announced by screen readers but not necessarily
  /// shown (design spec: dot for > 99).
...
/// Mobile bottom navigation — v2 design system.
///
/// Renders 2–5 equally-spaced destinations. Uses `surfaceCard` with a top
/// border for flat-aesthetic (§3.5) and SafeArea-aware bottom inset.
class GenaiBottomNav extends StatelessWidget {
  /// Destinations. Design rule: keep to 2–5.
  final List<GenaiBottomNavItem> items;

  /// Selected index; values outside the range select none.
  final int selectedIndex;

  /// Fires when a user activates a different destination.
  final ValueChanged<int>? onChanged;

  /// Accessibility override for the bar.
  final String? semanticLabel;
