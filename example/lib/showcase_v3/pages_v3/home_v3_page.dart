...
class _Agenda {
  ...
  const _Agenda(this.day, this.month, this.titolo, this.luogo, this.ora,
      this.tipo, this.tone);
}
...
class _HomeV3PageState extends State<HomeV3Page> {
  ...
  final _agenda = const [
    _Agenda(28, 'APR', 'Quiz Fondo Nuove Competenze',
        'Aula virtuale 3', '10:00', 'esame', GenaiChipTone.warn),
    _Agenda(30, 'APR', 'Webinar — AI nei flussi di lavoro',
        'Zoom', '14:30', 'lezione', GenaiChipTone.info),
    _Agenda(5, 'MAG', 'Revisione piano Obbligatoria', 'Sede Milano',
        '09:00', 'meeting', GenaiChipTone.info),
  ];
    ...
    return LayoutBuilder(
      builder: (ctx, cons) {
        ...
        return Wrap(
          ...
          runSpacing: gap,
          children: [
            for (final c in cards) SizedBox(width: cellW, child: c),
          ],
        );
      },
    );
  ...
  Widget _buildGrid2(BuildContext context) {
    ...
    return LayoutBuilder(
      builder: (ctx, cons) {
        ...
        if (!isWide) {
          return Column(
            children: [left, SizedBox(height: spacing.s14), right],
          );
        }
  ...
  Widget _buildChartCard(BuildContext context) {
    ...
    return GenaiCard.outlined(
      ...
      padding: EdgeInsets.symmetric(
          horizontal: context.spacing.s20, vertical: context.spacing.s14),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.stretch,
        children: [
        ...
        children: [
          for (var i = 0; i < _agenda.length; i++) ...[
            GenaiAgendaRow(
              ...
              meta: [
                GenaiChip.readonly(
                    label: _agenda[i].tipo, tone: _agenda[i].tone),
              ],
              onTap: () {},
            ),
